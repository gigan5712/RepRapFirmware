...
bool GCodes::HandleGcode(GCodeBuffer& gb, const StringRef& reply) THROWS(GCodeException)
{
	...
	case 4: // Dwell
		result = DoDwell(gb);
		break;
	case 8: // Start speed controlled extruder GG

		if (extruderSpeedCtrl == nullptr)
		{
			extruderSpeedCtrl = new ExtruderSpeedCtrl;

			extruderSpeedCtrl->Configure();
			extruderSpeedCtrl->Start();
		}

		if (gb.Seen('P'))
		{
			const uint16_t iScale = gb.GetUIValue();
			if( (iScale > 0) && (iScale < 101))
			{
				extruderSpeedCtrl->SetScale(iScale);
			}
			else
			{
				reply.printf("scaling out of range 1..100");
				break;
			}
		}

		reply.printf("ok - scaling set to: %i",extruderSpeedCtrl->GetScale());

		break;
	case 9: //Set constant speed

		if (extruderSpeedCtrl == nullptr)
		{
			reply.printf("initialize the extruder first");
		}
		else
		{
			if (gb.Seen('P'))
			{
				const uint16_t iSpd = gb.GetUIValue();

				// Set constant speed in mm/s
				extruderSpeedCtrl->SetConstantSpeed(iSpd);

				reply.printf("ok - speed set to %i",iSpd);
			}
...
# endif
		...
		case 43:	// GG: Report the state of a GPIO input
			{
				const uint32_t gpioPortNumber = gb.GetLimitedUIValue('P', MaxGpOutPorts);
				reply.printf("Input number: %.0f level  %.2f", (double)gpioPortNumber,(double)(reprap.GetPlatform().GetGpInPort(gpioPortNumber).GetState()));
			}
			break;
